...
gx = 0.0; gy = -100.0; rho1 = 1; rho2 = 2; mu = 0.01;    % parameters
...
figure(1); contourf(x,y,phi');  axis equal; axis([0 Lx 0 Ly]); 
hold on;axis equal; axis([0 Lx 0 Ly]); hold on; xlabel('x', 'Fontsize', 20);
ylabel('y', 'Fontsize', 20);
title([sprintf('time t=%0.3f', time)], 'Fontsize',20);
colorbar;%caxis([-1 1])
drawnow; hold off

figure(2); contourf(x,y,chi');  axis equal; axis([0 Lx 0 Ly]); hold on;
axis equal; axis([0 Lx 0 Ly]); hold on; xlabel('x', 'Fontsize', 20);
ylabel('y', 'Fontsize', 20);
title([sprintf('time t=%0.3f', time)], 'Fontsize',20);
colorbar;%caxis([-1 1])
drawnow; hold off
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for i=2:nx; for j=2:ny+1     % temporary u-velocity (boundary values are not touched)
    ut(i,j) = (2.0/(r(i+1,j)+r(i,j)))*(0.5*(ro(i+1,j)+ro(i,j))*u(i,j)+ dt* (...
            ...
            + 0.5*(ro(i+1,j)+ro(i,j))*gx + (fgx(i+1,j)+fgx(i,j))/2.0 ));
  end; end
  ...
  for i=2:nx+1; for j=2:ny       % temporary v-velocity (boundary values are not touched)
    vt(i,j) = (2.0/(r(i,j+1)+r(i,j)))*(0.5*(ro(i,j+1)+ro(i,j))*v(i,j)+ dt* (...     
            ...
            + 0.5*(ro(i,j+1)+ro(i,j))*gy + (fgy(i,j+1)+fgy(i,j))/2.0 ) );    
  end; end     
...
axis([0 Lx 0 Ly]); hold on; xlabel('x', 'Fontsize', 20);
ylabel('y', 'Fontsize', 20);
title([sprintf('time t=%0.3f', time)], 'Fontsize',20);
colorbar;
drawnow; hold off
...
for k=1:20; % iteration for 20 times
    psin=psi;
    for i=2:nx+1;
        for j=2:ny+1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
  ...
  axis equal;  %hold on;drawnow; hold off
  quiver(xh,yh,uu',vv','r'); axis([0 Lx 0 Ly]);hold on;
  axis equal; axis([0 Lx 0 Ly]); hold on; xlabel('x', 'Fontsize', 20);
 ylabel('y', 'Fontsize', 20);
 title([sprintf('time t=%0.3f', time)], 'Fontsize',20);
 colorbar;%caxis([-1 1])
